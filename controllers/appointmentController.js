...
export async function getAppointments(req, res) {
  try {
    ...
    return res.status(200).json({
      success: true, appointments: items, meta: { page, limit, total, count: items.length }
    });
  }
  catch (error) {
...
//     console.log("Resolved createdBy by ", resolvedCreatedBy);

//     if (!resolvedCreatedBy && !req.query.mobile) {
//       return res.status(400).json({ success: false, message: "createdBy or mobile is required" });
//     }
//     const filter = {};
  ...
  try {
    ...
    if (!resolvedCreatedBy && !req.query.mobile) {
      return res.status(400).json({
        success: false,
        message: "createdBy or mobile is required",
      });
    }
...
export const createAppointment = async (req, res) => {
  try {
    ...
    const clerkUserId = resolveClerkUserId(req);
    if (!clerkUserId) {
      return res.status(401).json({ success: false, message: "Unauthorized: User identification failed" });
    }
...
function parseDateTime(dateStr, timeStr) {
  ...
  return new Date(dateStr + "T" + String(hh).padStart(2, "0") + ":" + String(mm).padStart(2, "0"));
}
...
if (bookingDateTime < now) {
  return res.status(400).json({
    ...
    if (existingBooking) {
      return res.status(409).json({ success: false, message: "An appointment for this doctor at the specified time already exists." });
    }
    let doctor = null;
    try {
...
export async function updateAppointment(req, res) {
  try {
    ...
    const updated = await Appointment.findByIdAndUpdate(id, update,
      { new: true, runValidators: true }).populate({ path: "doctorId", select: "name imageUrl" }).lean();
...
export const getStats = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, stats: { total, revenue, recentLast7Days: recent } });
  }
  catch (error) {
