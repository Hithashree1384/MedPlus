...
const appointmentDateTime = new Date(
  `${date} ${finalHour}:${String(finalMinute).padStart(2, "0")} ${finalAmpm}`
);
...
//confirm
export const confirmServivePayment=async(req,res)=>{
    try{
        const {session_id} =req.query;
        if(!session_id) return res.status(400).json({
            success:false,
            message:"Stripe is not recognized"
        });
        let session;
        try{
    ...
    return res.json({success:true,appointment:appt})
        ...
        console.error("Confirm service payment error",error);
        return res.status(500).json({success:false,message:"server error"})
...
export const getServiceAppointmentById=async (req,res)=>{
    try{
        ...
        if(!appt) return res.status(404).json({success:false,message:"Not found appointment"});

        return res.json({success:true,data:appt});
    }
    catch(error){
        console.error("Appointment by id  error",error);
        return res.status(500).json({success:false,message:"server error"})
...
export const cancelServiceAppointment=async(req,res)=>{
    ...
    return res.json({success:true,data:appt})
        ...
        console.error("Cancel service Appointment error",error);
        return res.status(500).json({success:false,message:"server error"})
...
export const getServiceAppointmentStats=async (req,res)=>{
    ...
    console.error("Getservice Appointment stats error",error);
    return res.status(500).json({success:false,message:"server error"})
...
export const getServiceAppointmentByPatient=async (req,res)=>{
    try{
        ...
        const list=await ServiceAppointment.find(filter).sort({createdBy:-1}).lean();
        return res.json({
            success:true,
            data:list
        })
        ...
        console.error("Getservice Appointmentby patient error",error);
    return res.status(500).json({success:false,message:"server error"})
